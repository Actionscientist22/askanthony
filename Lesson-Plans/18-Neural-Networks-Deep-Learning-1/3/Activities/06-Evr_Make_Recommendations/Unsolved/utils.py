...
# Function to import new ratings data
def get_new_data():
    df = pd.read_csv("https://static.bc-edx.com/ai/ail-v-1-0/m18/lesson_3/datasets/new-users.csv")
    df = df.drop_duplicates(subset=["user_id", "movie_id"], keep="last")

    return df

# Function to normalize the new ratings data
def get_normalized_new_data():
    
    df = get_new_data()

    ratings_matrix = pivot_data()
    pivot_new_data = pd.DataFrame(columns=ratings_matrix.columns)
    for index, row in df.iterrows():
        pivot_new_data.loc[int(row["user_id"]), row["movie_id"]] = row["rating"]

    pivot_new_data = pivot_new_data.fillna(0)
    
    normalized_ratings = normalize_data(pivot_new_data)
